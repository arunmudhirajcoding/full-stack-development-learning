...
// imp methods filter, map , some , any, all
let arr2 = [10,20,30,11,21,44,51];

let evenArray = arr2.filter((number) => { // return values which are true
...
let arr3 = [1, 2, 3, "arun", 4, 2, "tharun"];

let ans = arr3.filter((num) => {
...
let arrk = [9, 1, 7, 4, 2, 8];

arrk.map((number,index) => {
    console.log(number);
    console.log(index);
})

let ansArray  = arrk.map((number) => {
...
let nrr = [1,2,'love','kunal', null];

let ans3 = nrr.filter((value) => {
...
arr.sort((a,b)=>{
    return a-b //compare each element
});
console.log(arr);
...
const a = Array(5); // creats a 5 lenght mt items array
console.log(a);
a.push(1);
a[0] = 3;
a[3] = 10;
